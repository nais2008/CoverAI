...
class Block(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.group_norm = ConditionalGroupNorm(
            norm_groups, in_channels, time_embed_dim
        )
        ...
        self.resnet_blocks = nn.ModuleList(
            [
                ...
                for (
                    in_channel,
                    out_channel,
                ), kernel_size, up_resolution in zip(
...
class AttentionPolling(nn.Module):
    ...
    def __init__(self, num_channels, context_dim, head_dim=64):
        ...
        self.attention = Attention(
            context_dim, num_channels, context_dim, head_dim
        )
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.in_norm = ConditionalGroupNorm(
            norm_groups, num_channels, time_embed_dim
        )
        self.attention = Attention(
        ...
        self.out_norm = ConditionalGroupNorm(
            norm_groups, num_channels, time_embed_dim
        )
        self.feed_forward = nn.Sequential(
            nn.Conv2d(
                num_channels, hidden_channels, kernel_size=1, bias=False
            ),
            nn.SiLU(),
            nn.Conv2d(
...
class UpSampleBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        up_resolutions = [
            [None, set_default_item(up_sample, True), None, None]
        ] + [[None] * 4] * (num_blocks - 1)
        hidden_channels = (
...
class ControlNetModel(nn.Module):
    def __init__(
        ...
        self.in_layer = nn.Conv2d(
            num_channels, init_channels, kernel_size=3, padding=1
        )
        ...
        text_dims = [
            set_default_item(is_exist, context_dim)
    ...
    def forward(self, x, time, context=None, context_mask=None):
        ...
        if exist(context):
            time_embed = self.feature_pooling(
                time_embed, context, context_mask
            )
...
class UNet(nn.Module):
    ...
    def forward(
        ...
        for level, down_sample in enumerate(self.down_samples):
            x = down_sample(
                x, time_embed, context, context_mask, control_net_residual
            )
            if level != self.num_levels - 1:
                hidden_states.append(x)
...
class ControlUNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.control_net = ControlNetModel(
            model_channels,
            init_channels,
    ...
    def forward(
        self, x, time, context=None, context_mask=None, control_net_data=None
    ):
        time_embed = self.to_time_embed(time)
        if exist(context):
            time_embed = self.feature_pooling(
        ...
        control_net_hiddens = self.control_net(
            control_net_data, time, context, context_mask
        )
        hidden_states = []
        x = self.in_layer(x)
        for level, down_sample in enumerate(self.down_samples):
