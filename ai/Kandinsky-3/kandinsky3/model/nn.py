...
class Attention(nn.Module):
    ...
    def forward(self, x, context, context_mask=None):
        ...
        key = rearrange(
            self.to_key(context), "b n (h d) -> b h n d", h=self.num_heads
        )
        value = rearrange(
            self.to_value(context), "b n (h d) -> b h n d", h=self.num_heads
        )

        attention_matrix = (
            einsum("b h i d, b h j d -> b h i j", query, key) * self.scale
        )
