...
class BaseDiffusion:
    ...
    def get_x_start(self, x, t, noise):
        ...
        sqrt_alphas_cumprod = get_tensor_items(
            self.sqrt_alphas_cumprod, t, noise.shape
        )
        pred_x_start = (
            x - sqrt_one_minus_alphas_cumprod * noise
        ) / sqrt_alphas_cumprod
        return pred_x_start
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        posterior_mean = (
            posterior_mean_coef_1 * x_start + posterior_mean_coef_2 * x_t
        )

        posterior_variance = get_tensor_items(
    ...
    def q_posterior_variance(
        ...
        prev_alphas_cumprod = get_tensor_items(
            self.alphas_cumprod, prev_t, shape
        )

        posterior_variance = torch.sqrt(
            eta
            * (1.0 - alphas_cumprod / prev_alphas_cumprod)
            * (1.0 - prev_alphas_cumprod)
            / (1.0 - alphas_cumprod)
        )
    ...
    def text_guidance(
        ...
        large_context_mask = torch.cat(
            [context_mask, uncondition_context_mask]
        )
    ...
    def p_mean_variance(
        ...
        pred_mean += (
            torch.sqrt(1.0 - prev_alphas_cumprod - pred_var**2) * pred_noise
        )
    ...
    def p_sample_loop(
        ...
        for time, prev_time in tqdm(times):
            ...
            if gan:
                ...
                pred_noise = model(
                    x_t, time.type(x_t.dtype), context, context_mask.bool()
                )
