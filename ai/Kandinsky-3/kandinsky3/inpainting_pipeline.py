    ...
    def prepare_batch(
        ...
        batch = {
            "image": self.to_tensor(
                resize_image_for_diffusion(image.convert("RGB"))
            )
            * 2
            - 1,
        ...
        batch["image"] = (
            batch["image"].unsqueeze(0).to(self.device_map["movq"])
        )
        ...
        if negative_condition_model_input is not None:
            batch["negative_text"]["input_ids"] = batch["negative_text"][
                "input_ids"
            ].to(self.device_map["text_encoder"])
            batch["negative_text"]["attention_mask"] = batch["negative_text"][
                "attention_mask"
            ].to(self.device_map["text_encoder"])
        ...
        with torch.no_grad():
            ...
            for minibatch in [bs] * k + [m]:
                ...
                for images_chunk in images.chunk(1):
                    pil_images += [
                        self.to_pil(image) for image in images_chunk
                    ]
