...
class T5TextConditionProcessor:
    ...
    def encode(self, text=None, negative_text=None):
        encoded = self.processor(
            text, max_length=self.tokens_length, truncation=True
        )
        pad_length = self.tokens_length - len(encoded["input_ids"])
        input_ids = (
            encoded["input_ids"] + [self.processor.pad_token_id] * pad_length
        )
        ...
        if negative_text is not None:
            ...
            negative_input_ids = (
                negative_input_ids
                + [self.processor.pad_token_id] * negative_pad_length
            )
            negative_attention_mask = (
                encoded["attention_mask"] + [0] * pad_length
            )
            negative_condition_model_input = {
                "input_ids": torch.tensor(
                    negative_input_ids, dtype=torch.long
                ),
